$(document).ready(function() {
    ...
    $('.skip-btns span').click(function(){
    $('.skip-sure').css('visibility', 'visible');
    });
    ...
    var areas = ["lorem", "ipsum", "fitsum", "etc" ]

    var selectizeAreas = areas.map(function(x) { return { area: x }; });
    var selectizeCountries = countries.map(function(x) { return { country: x }; });

    $('#areas').selectize({
        sortField: 'text',
        options: selectizeAreas,
        maxItems: 1,
        searchField: ['area'],
        labelField: 'area',
        valueField: 'area'
